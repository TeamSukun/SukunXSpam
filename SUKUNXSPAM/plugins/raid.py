...
from resources.data import RAID, REPLYRAID, SUKUNXSPAM, BRTHSPAM
from SUKUNXSPAM import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9, SUDOERS
...
async def spam(e):
    ...
    if e.sender_id in SUDOERS:
        ...
        Sukun = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        bitxh = await e.get_reply_message()
        if len(Sukun) == 2:
            user = str(Sukun[1])
            a = await e.client.get_entity(user)
            g = a.id
            if int(g) in SUKUNXSPAM:
                text = f"» ᴄᴀɴɴᴏᴛ ʀᴀɪᴅ ᴏɴ sᴜᴋᴜɴxsᴘᴀᴍ ᴄʀᴇᴀᴛᴏʀ !"
                await e.reply(text)
...
async def _(event):
   ...
   if event.sender_id in SUDOERS:
         ...
         Sukun = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(Sukun) == 3:
             user = str(Sukun[2])
             a = await event.client.get_entity(user)
             e = a.id
             if int(e) in SUKUNXSPAM:
                 text = f"**» ᴄᴀɴɴᴏᴛ ʀᴀɪᴅ ᴏɴ sᴜᴋᴜɴxsᴘᴀᴍ ᴄʀᴇᴀᴛᴏʀ !**"
                 await event.reply(text)
