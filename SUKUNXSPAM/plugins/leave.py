...
async def _(event):
    ...
    if event.sender_id in SUDOERS:
        sukun = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
        if len(event.text) > 7:
            bc = sukun[0]
            Xd = int(bc)           
            TeamSukunreply = await event.reply("**ʟᴇᴀᴠᴇ ᴄᴏᴍᴍᴀɴᴅ ʀᴇᴄᴇɪᴠᴇᴅ 🥺**")
            try:
                await event.client(LeaveChannelRequest(Xd))
                await TeamSukunreply.edit("**» ꜱᴘᴀᴍᴍᴇʀꜱ ʟᴇғᴛ ᴛʜᴇ ᴄʜᴀᴛ**")
